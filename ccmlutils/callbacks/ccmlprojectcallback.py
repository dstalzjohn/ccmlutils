...
def produce_project_info(filepath: str, exp_prefix: str or None):
    out_dict = {envc.RUN_ID_KEY: envc.get_run_id(), envc.SHORT_ID_KEY: envc.get_short_id(),
                envc.EXP_NAME_KEY: envc.get_exp_name(), envc.EXP_PREFIX_KEY: exp_prefix}
...
class CCMLProjectCallback(Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        produce_project_info(join(self.log_path, self.project_info_filename), exp_prefix=self.exp_prefix)
        if self.log_callback is not None:
